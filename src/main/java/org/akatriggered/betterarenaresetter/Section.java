            ...
            while (resetCurrentTypeIndex < amount)
            {
                ...
                if (NBT_CACHE.containsKey(resetLocationIndex)) { //If the NBT cache has this location
                    if (!(block.getState() instanceof TileState)) { //If the placed block doesn't have NBT somehow???
                        BetterArenaResetter.INSTANCE.getLogger().warning("Tried to place NBT at block " + block.toString() + " but can't (no TileState)");
                    } else {
                        ...
                        if (!wrapper.getTileClass().isAssignableFrom(block.getState().getClass())) { //If the blocktype doesnt match
                            BetterArenaResetter.INSTANCE.getLogger().warning("Tried to place NBT at block " + block.toString() + " but can't (TileState type mismatch)");
                            BetterArenaResetter.INSTANCE.getLogger().warning("Block '" + block.getState().getClass().getName() + "', wrapper '" + wrapper.getTileClass().getName() + "'");
                        } else {
                            try {
                                wrapper.place((TileState) block.getState(), cache).update(); //The update part is important or else the tilestate won't change
                            } catch (Exception e) {
                                BetterArenaResetter.INSTANCE.getLogger().warning("Tried to place NBT at block " + block.toString() + " and failed");
                                e.printStackTrace();
                            }
                        }
                    }
