...
import org.akatriggered.betterarenaresetter.Util;
...
public class ListCommand extends ArenaCommand {
    ...
    public void execute(CommandSender sender, List<String> args) {
        ...
        for (Arena arena : Arena.arenas.values()) {
            ...
            for (BaseComponent c : TextComponent.fromLegacyText(BetterArenaResetter.PREFIX + string)) arenaComponent.addExtra(c);
        ...
        if (page < 1 || page > arenas.size() / 10 + 1) {
            sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " Invalid page number!");
            return;
        }

        sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " Listing all the arenas: ");
        for (int i = page * 10 - 10; i < page * 10 && i < arenas.size(); i++) {
            //sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " - " + arenas.get(i));
            sender.spigot().sendMessage(arenas.get(i));
        }

        if (page + 1 <= arenas.size() / 10 + 1) { //If there is a next page
            sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.YELLOW + " To view the next page, type /arena list " + (page + 1));
        }
    }
