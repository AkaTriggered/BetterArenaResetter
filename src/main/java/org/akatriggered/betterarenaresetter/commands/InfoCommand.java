...
public class InfoCommand extends ArenaCommand {
    ...
    public void execute(CommandSender sender, List<String> args) {
        if (!sender.hasPermission("BetterArenaResetter.info")) {
        ...
        if (args.size() == 0) {
            sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " Command usage is /arena info <arena>");
            return;
        }
        ...
        sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " === " + ChatColor.YELLOW + arena.getName() + ChatColor.RED + " ===");
        ...
        if (time == 0) {
            File arenaFolder = new File(BetterArenaResetter.INSTANCE.getDataFolder(), "Arenas");
        ...
        for (String line : s.split("\n")) {
            sender.sendMessage(BetterArenaResetter.PREFIX + " " + line);
        }
    }
