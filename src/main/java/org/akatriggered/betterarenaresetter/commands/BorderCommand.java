...
import org.akatriggered.betterarenaresetter.Section;
...
public class BorderCommand extends ArenaCommand {
    ...
    public void execute(CommandSender sender, List<String> args) {
        if (!sender.hasPermission("BetterArenaResetter.list")) {
        ...
        if (args.size() == 0) {
            if (borders.containsKey(sender)) {
                ...
                sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " Border visibility disabled.");
                return;
            }
            sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " You must provide an arena to show borders on!");
            return;
        }
        ...
        if (borders.containsKey(sender)) {
            sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " Border visibility of arena \"" + borders.get(((Player)sender)).getName() + "\" disabled.") ;
            removePlayer(borders.get(sender), ((Player)sender));
            return;
        }

        Arena arena = Arena.arenas.get(args.get(0).toLowerCase());
        sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " Showing the border of arena \"" + arena.getName() + "\"! Run /arena hideborders to disable it");
        ...
        }.runTaskLater(BetterArenaResetter.INSTANCE, 20 * 60 * 2);
    }
    ...
    public static void sendBorder(Arena arena, Player player, boolean revert) {
        ...
        for (int i = 0; i < runnables.size(); i++) {
            runnables.get(i).runTaskLater(BetterArenaResetter.INSTANCE, i + 1);
        }
    }
