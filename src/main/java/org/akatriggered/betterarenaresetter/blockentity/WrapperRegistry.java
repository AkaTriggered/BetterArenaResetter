package org.akatriggered.betterarenaresetter.blockentity;
...
public class WrapperRegistry {
    ...
    private static Material[] getSigns() {
        ...
        if (BetterArenaResetter.getMCVersionInt() >= 1160) woods.addAll(Arrays.asList("CRIMSON", "WARPED"));
        if (BetterArenaResetter.getMCVersionInt() >= 1190) woods.add("MANGROVE");
        if (BetterArenaResetter.getMCVersionInt() >= 1193) woods.add("BAMBOO");

        Set<String> types = new HashSet<>(Arrays.asList("SIGN", "WALL_SIGN"));
        if (BetterArenaResetter.getMCVersionInt() >= 1193) types.addAll(Arrays.asList("HANGING_SIGN", "WALL_HANGING_SIGN"));
