package org.akatriggered.betterarenaresetter;

import org.akatriggered.betterarenaresetter.blockentity.Wrapper;
import org.akatriggered.betterarenaresetter.blockentity.WrapperRegistry;
import org.akatriggered.betterarenaresetter.commands.DebugCommand;
...
public class Arena {
    ...
    protected void addKeys(Collection<BlockData> keys) {
        ...
        //BetterArenaResetter.INSTANCE.getLogger().info("Key took " + (System.currentTimeMillis() - time) + "ms");
    }
    ...
    private void loopyReset(ResetLoopinData data, CommandSender sender) {
        ...
        if (data.sections.size() == 0) {
            ...
            if (sender != null && (!(sender instanceof Player) || ((Player)sender).isOnline()) && !data.silent) {
                ...
                sender.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " Arena \"" + getName() + "\" reset complete (took " + tookS + ")!");
            }
        ...
        }.runTaskLater(BetterArenaResetter.INSTANCE, 1L);
    }
    ...
    public static Arena createNewArena(String name, Location corner1, Location corner2, Player player) {
        ...
        player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " Creating a new arena named \"" + name + "\"...");
        ...
        for (int sx = 0; sx < sectionsX; sx++) {
            for (int zx = 0; zx < sectionsZ; zx++) {
                ...
                //BetterArenaResetter.INSTANCE.getLogger().info("Debug: " + start.toString() + ", " + end.toString());

            }
        }

        String totalSize = NumberFormat.getInstance().format(width * length * height);
        BetterArenaResetter.INSTANCE.getLogger().info("Creating new arena. Size is " + width + " x " + height + " x " + length + " and totals " + totalSize + " blocks.");

        player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " Arena size is " + ChatColor.YELLOW + width + " x " + height + " x " + length + ChatColor.GREEN + " and totals " + ChatColor.YELLOW + totalSize + " blocks.");
        if (sectionsX * sectionsZ > 1) player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " " + (sectionsX * sectionsZ) + " sections will be created.");
        player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " Now analyzing individual blocks... (This may take a while)");
        ...
        arena.setMcVersion(BetterArenaResetter.INSTANCE.getMCVersion()); //We have to set these values now so the arena can be used in the info command
        ...
        Runnable runnable = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (player != null && player.isOnline()) {
                    player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " Done analyzing! Took " + tookS + " over " + data.tick + " ticks.");
                    player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.YELLOW + " Saving to disk...");
                }

                Arena.arenas.put(arena.name, arena);
                String extension = ConfigManager.ENABLE_COMPRESSION ? ".datc" : ".dat";
                ArenaIO.saveArena(new File(BetterArenaResetter.INSTANCE.getDataFolder(), "/Arenas/" + name + extension), arena);

                if (player != null && player.isOnline()) {
                    player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.GREEN + " Done! The arena is now ready for use!");
                }
                BetterArenaResetter.INSTANCE.getLogger().info("Arena created in " + tookS + " over " + data.tick + " ticks.");
            }
        };
    ...
    private static void loopyCreate(CreationLoopinData data, final int amount, Player player, Runnable onFinished) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            try {
                ...
                e.printStackTrace();
                BetterArenaResetter.INSTANCE.getLogger().info("Debug for error: " + width + " x " + height + " x " + length);
                BetterArenaResetter.INSTANCE.getLogger().info("Debug for error2: " +start + " | " + end + " || " + data.arena.getCorner1() + " | " + data.arena.getCorner2());
                return;
            }

            loc = start.clone().add(loc);
            if (data.index >= width * height * length) {
                //BetterArenaResetter.INSTANCE.getLogger().info("Debug3: We are at the end");
            ...
            if (!keyList.contains(loc.getBlock().getBlockData())) {
                ...
                if (keyList.size() == Short.MAX_VALUE) {
                    player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " Oh bother. It looks like you've used up all " + Short.MAX_VALUE + " block states that can be stored in an arena file.");
                    player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " This means that there is too many types of blocks in this arena to safely store. Are you running modded minecraft?");
                    player.sendMessage(BetterArenaResetter.PREFIX + ChatColor.RED + " For now, all you can do is create a smaller arena with less than " + Short.MAX_VALUE + " different block states.");
                    try {
                ...
                //BetterArenaResetter.INSTANCE.getLogger().info("Debug2: Adding " + loc.getBlock().getBlockData().getAsString() + " to keylist");
            }

            short blockKeyIndex = (short)keyList.indexOf(loc.getBlock().getBlockData());
            //BetterArenaResetter.INSTANCE.getLogger().info("Debug6: Index of " + loc.getBlock().getBlockData().getAsString() + " is " + index + "(" + keyList.indexOf(loc.getBlock().getBlockData()) + ")");

            if (data.blockTypes.length == 0) {
                data.blockAmounts = new short[] {1};
                data.blockTypes = new short[] {blockKeyIndex};
                //BetterArenaResetter.INSTANCE.getLogger().info("Debug4: Creating array thingies");
                if (keyList.size() > data.arena.keys.length) data.arena.addKeys(keyList);
                data.totalBlocks++;
                data.index++;
            ...
            if (data.blockTypes[data.blockTypes.length - 1] == blockKeyIndex) { //If the last block recorded is the same type
                ...
                //BetterArenaResetter.INSTANCE.getLogger().info("Debug5: Same type. Amount is now " + data.blockAmounts[data.blockAmounts.length - 1]);
                if (keyList.size() > data.arena.keys.length) data.arena.addKeys(keyList);
                data.index++;
                data.totalBlocks++;
                updatePlayerCreate(player, data);
        ...
        data.tick++;
        if (keyList.size() > data.arena.keys.length) data.arena.addKeys(keyList);
        //BetterArenaResetter.INSTANCE.getLogger().info("Debug5: new tick time");
    ...
    private static void updatePlayerCreate(Player player, CreationLoopinData data) {
        if (System.currentTimeMillis() - data.lastUpdate > 10 * 1000 && !data.silent) {
            ...
            BetterArenaResetter.INSTANCE.getLogger().info("Arena " + percS + " analyzed (" + NumberFormat.getInstance().format(data.totalBlocks) + " blocks)");
            data.lastUpdate = System.currentTimeMillis();
        }
    }
    ...
    public boolean hasOwner() {
        return !creator.equals(BetterArenaResetter.DEFAULT_OWNER);
    }
