...
public class ConfigManager {
    ...
    public static boolean setup() {
        File file = new File(org.akatriggered.betterarenaresetter.BetterArenaResetter.INSTANCE.getDataFolder(), "config.yml");
        if (!file.exists()) {
            if (!org.akatriggered.betterarenaresetter.Util.saveResource("config.yml", file)) {
                org.akatriggered.betterarenaresetter.BetterArenaResetter.INSTANCE.getLogger().severe("Failed to copy default config!");
                return false;
            }
        }
        ...
        try {
            ...
            org.akatriggered.betterarenaresetter.BetterArenaResetter.INSTANCE.getLogger().severe("Failed to load config.yml!");
            e.printStackTrace();
            return false;
        } catch (InvalidConfigurationException e) {
            org.akatriggered.betterarenaresetter.BetterArenaResetter.INSTANCE.getLogger().severe("Invalid YML format used in config.yml!");
            e.printStackTrace();
            return false;
        }
    }
