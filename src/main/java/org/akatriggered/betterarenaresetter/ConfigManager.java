...
public class ConfigManager {
    ...
    public static boolean setup() {
        File file = new File(etterarenaresetter.INSTANCE.getDataFolder(), "config.yml");
        if (!file.exists()) {
            if (!Util.saveResource("config.yml", file)) {
                etterarenaresetter.INSTANCE.getLogger().severe("Failed to copy default config!");
                return false;
            }
        }
        ...
        try {
            ...
            etterarenaresetter.INSTANCE.getLogger().severe("Failed to load config.yml!");
            e.printStackTrace();
            return false;
        } catch (InvalidConfigurationException e) {
            etterarenaresetter.INSTANCE.getLogger().severe("Invalid YML format used in config.yml!");
            e.printStackTrace();
            return false;
        }
    }
